...
#define HYPRE_FACTORY_TYPE HYPRE_FACTORY##_t
...
#define STR(s) SSTR(s)
#define SSTR(s) #s
HYPRE_FACTORY_TYPE* HYPRE_FACTORY(void);
HYPRE_FACTORY_TYPE* HYPRE_FACTORY(void)
{
  HYPRE_FACTORY_TYPE* factory = polymec_refcounted_malloc(sizeof(HYPRE_FACTORY_TYPE),
                                                          factory_free);
  factory->hypre_libs = ptr_array_new();
  factory->hypre_lib_names = string_array_new();
  factory->hypre_ext = NULL;

  // Make a list of all the HYPRE libraries in our search paths.
  string_array_t* all_hypre_libs = string_array_new();
  {
    ...
    {
      log_debug(STR(HYPRE_FACTORY) ": Looking for HYPRE libraries in %s.", hypre_dir);
      {
        ...
        while (node != NULL)
        {
          ...
          {
            string_array_append_with_dtor(all_hypre_libs, string_dup(node->value), string_free);
          }
          node = node->next;
        }
  ...
  {
    log_urgent(STR(HYPRE_FACTORY) ": Didn't find any HYPRE libraries!");
    goto failure;
  }

  log_debug(STR(HYPRE_FACTORY) ": Found %d HYPRE libraries.", (int)all_hypre_libs->size);
  ...
  { \
    ...
    { \
      ...
      { \
        ...
        void* lib = polymec_dlopen(all_hypre_libs->data[ilib]); \
        if (lib != NULL) \
        { \
          ...
          { \
            ...
            ptr_array_append_with_dtor(factory->hypre_libs, lib, polymec_dlclose); \
            break; \
          } \
          else \
            polymec_dlclose(lib); \
